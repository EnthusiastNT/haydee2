...
//  - shout out to Coltaho and his Timespinner.asl, was extremely helpful!
...
startup {
	...
	vars.myAddItem = (Func<string, bool, string, string, bool>)((sId, bDefault, sDescription, sTooltip) => {
		// if we add things twice add a marker
		//if ( vars.addMarker )
		//	sId = "prctc_" + sId;
...
/*
	//settings.Add("StartOnLoad", true, "auto-start on load from a save...");  // doesn't work
	//settings.SetToolTip("StartOnLoad", "note: only works if timer was reset");

	settings.CurrentDefaultParent = null;
	settings.Add("Full_Run", true, "Full Run");

		settings.CurrentDefaultParent = "Full_Run";
		...
		settings.SetToolTip("Rooms", "(let me know if you need more...)");
		settings.CurrentDefaultParent = "Rooms";
			...
			vars.myAddItem("N7_ADM_Elevator", true, "ADM Elevator (the actual elevator)", "");
		...
		// may not make sense to split on but we use it to verify slots
		settings.CurrentDefaultParent = "Full_Run";
	...
	// practice shorter sections of 10-12 minutes each
	settings.CurrentDefaultParent = null;
	settings.Add("Practice_Run", false, "Practice (disable Full Run!)");
	settings.SetToolTip("Practice_Run", "load from a save and start the timer manually");
	//vars.addMarker = true;
*/
	...
	settings.CurrentDefaultParent = null;
	settings.Add("FromStart", true, "Pistol to Boltcutter, Medical save (9)");
	settings.SetToolTip("FromStart", "if you disable this also auto-skip 1+8=9");
	settings.CurrentDefaultParent = "FromStart";
		...
		vars.myAddItem("Wrench", true, "Wrench", "");
		vars.myAddItem("N7_ENG_Arena", true, "ENG Arena (above arena)", "");
// drop 2 ammo at Technical L1 stairs
// throw things in Medical (including Screw and Wrench)
		vars.myAddItem("CardOrange", true, "Orange", "");
		vars.myAddItem("Clicker", true, "Wifi Clicker", "remote control");
		vars.myAddItem("JackGrip", true, "Lever", "Jack, long lever grip for forklift");
		vars.myAddItem("Boltcutter", true, "Boltcutter", "");
// drop Clicker in front of door to Wifi button (take before Hammer)

	settings.CurrentDefaultParent = null;
	settings.Add("FromMedical", true, "Crowbar to Security save (6)");
	settings.SetToolTip("FromMedical", "if you disable this also auto-skip 1+2+4+8=15");
	...
	settings.CurrentDefaultParent = null;
	settings.Add("FromSecurity", true, "Blue to Engineering save (5)");
...
// take Receiver here
	...
	settings.CurrentDefaultParent = "beforeHammer";
		vars.myAddItem("Hammer", true, "Hammer", "");
// drop 2 ammo at Ventilation save
		vars.myAddItem("Mask", true, "Gas Mask", "Breather");
		vars.myAddItem("FlaskC", true, "red vial", "");
		vars.myAddItem("FlaskB", true, "blue vial", "");
		vars.myAddItem("FlaskAgent", true, "White Antidote", "Agent, white vial/flask/tube");
...
*/
	...
	settings.CurrentDefaultParent = "info";
		settings.Add("info1", false, "Haydee 2 Autosplitter v1.1 (by Enthusiast)");
...
/*				if ( settings["Full_Run"] ) {
					// not those with "prctc_" marker
					if ( !item.StartsWith("prctc_") )
						vars.itemsToFind[item] = 1;
				} else if ( settings["Practice_Run"] ) {
					// ONLY those with "prctc_" marker (but remove that)
					if ( item.StartsWith("prctc_") ) {
						string itemClean = item.Replace("prctc_", "");
						vars.itemsToFind[itemClean] = 1;
					}
				}
*/				
			}
