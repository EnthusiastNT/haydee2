...
//   (shoutout to Coltaho, his Timespinner.asl was quite helpful!)

// patch 1.13, manifest 1 December 2021 (basically 1.12-FE0)
state("SteamGame", "1.13")
{
	...
	bool ppStarted : "SteamGame.exe", 0x827370;  // 828350-FD8=827378			PROBABLY

	// 0x01 when loading
	bool ppLoading : "SteamGame.exe", 0x8311EC;  // 8321CC-FE0=8311EC			PROBABLY

	// in-game time as float (4 bytes)
	float ppIngameTime : "SteamGame.exe", 0x83128C;  // 83226C-FE0=83128C		OKAY

	// this seems to point to the current room, 0 before start, 0 after end, 0 when loading
	long ppRoomAddr : "SteamGame.exe", 0x8312A8;  // 832288-FE0=8312A8			PROBABLY

	// room name, unicode 64 bytes = 32 chars
	string64 ppRoomName : "SteamGame.exe", 0x832EEE;  // 833EC0-FD2=832EEE		OKAY

	// number of saves
	int ppNumSaves : "SteamGame.exe", 0x8311D4;  // 8321AC-FD8=8311D4			OKAY
}

// patch 1.12, depot 24 August 2021 (basically 1.11+7160)
...
// patch 1.11, depot 13 April 2021 (basically 1.10+1070)
...
*/
	...
	settings.CurrentDefaultParent = "info";
		...
		settings.Add("info2", false, "(supports patches 1.07 to 1.13)");
		settings.Add("info3", false, "Split-file with icons available on GitHub:");
...
init {
	...
	switch (MD5Hash) {
		...
		case "1269D7F3B6951A5D4AC1EBE4D945EBEB":
			...
			break;
		case "48E5EF3220B7CBB9FDF262505FD417D5":
		default:
			version = "1.13";
			vars.SlotBase = 0x831550;  // 832530-FE0=831550		OKAY
			break;
	}
...
split {
	...
	if ( !vars.bShuttleEntered && !String.IsNullOrEmpty(vars.curRoomName) && vars.curRoomName.Contains("N7_SEC_Shuttle") ) {
		print("HDHDHD (yes, Shuttle entered)");
